...
studying. Entire universe of cases. 

**Parameter**: a numerical quantity derived from the population
(almost always unknown)

**Sample**: a subset of our population of interest. The observations included in the analysis. 
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-SkylarGMontague/manhattan.csv")
```
...
Here we are generating 10,000 samples. 
...
What does this mean?
In our repeated samples, we are 95% confident (or 19 out of 20 times) that our interval contains the true mean of the population. 
Does not tell us where in our confidence interval. 
...
What does this mean?
we are 95% sure that the mean of mice that died while not on treatment is between 69% and 79.3%
**Interpretation**:
...
```{r practice_2_ci}
ci_mice_lived <- get_ci(boot_prop, level = .9)
ci_mice_lived
```
...
We are 90% sure that the mean of mice that lived while on treatment is between 55.2% and 65.1%. 
